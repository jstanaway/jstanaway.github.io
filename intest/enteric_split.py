...
def main(loc):
    ...
    is_high_burden, is_codem = get_source_categories(loc, level_3)
    ...
    if level_3 == 'intest':
        ...
        pr_para = interpolate_draws(pr_para, meids['pr_para'], pr_para_model_version, release, 'pr_cause', 'logit')
    ...
    if is_codem:
        if level_3 == 'intest':
            codem_model_ids = [get_latest_dr_codem_models(cids['typhoid'], 1, release), get_latest_dr_codem_models(cids['typhoid'], 2, release),
                               get_latest_dr_codem_models(cids['paratyphoid'], 1, release), get_latest_dr_codem_models(cids['paratyphoid'], 2, release)]
            ...
            codem_model_ids = [get_latest_dr_codem_models(cids['ints_hiv'], 1, release), get_latest_dr_codem_models(cids['ints_hiv'], 2, release)]
            
            cod_draws = pd.concat([get_cod_draws(cids['ints_hiv'], codem_model_ids[0]),
                                   get_cod_draws(cids['ints_hiv'], codem_model_ids[1])])
...
def get_source_categories(loc, level_3):
    ...
    # Category is based on super-region, with two exceptions: 
    ...
    is_high_burden = (((loc_meta.super_region_id.isin([4, 158, 166, 137])) \
    ...
    return is_high_burden, is_codem
...
"""
Context: We need to pull DisMod estimates and reshape to long (much easier and faster
to work with)

Purpose: Pull DisMod model draws, reshape to long
...
def split_india(df, in_loc):
    ...
    df = df.join(ind_split, on = ['year_id', 'age_group_id', 'sex_id'], how = 'outer_coalesce')
...
def split_sequelae(df, cod = None, cod_sequela = None, epi_years = True, value_name = 'incidence', prop = None, prop_value_name = 'pr_cause'):
    ...
    if prop is not None:
        ...
        prop.rename({prop_value_name: value_name})
