...
# Modeler needs to input these four values
#RELEASE = 16
#MODEL_VERSION = 733758 #799167 #799169 #799168 #719331 # 
#MEID = 10139 #1252 # 10140 # 
#LOC = 161
...
def get_model_metadata(MODEL_VERSION):
    ...
    db = pymysql.connect(host = 'modeling-epi-db.ihme.washington.edu', user = 'dbview', password = 'E3QNSLvQTRJm', database = 'epi') 
...
#template = combine_covariate_estimates(template, model_data, covar_data)
#template.head()
...
def fill_time_series(df, ref_years, measure_id):
    ...
    def fts_main(df, ref_years, measure_id):
        ...
        # Let's only print messages once per location, so only for a single age/sex combo -- see if we're printing here
        #to_print = (int(df['age_group_id'].mean()) == 10 and
        #            int(df['sex_id'].mean()) == 1 and
        #            int(df['location_id'].mean()) == 7)
        ...
        if 'lincom' not in df.keys():
            #if to_print:
            #    print('NO COVARIATE EFFECTS FOUND -- INTERPOLATING BASED PURELY ON YEAR!!!')
            status_lincom = False    
            df['lincom'] = 0
        else:
            status_lincom = True
            #if to_print:
            #    print('Covariate effects found')
        ...
        if measure_id == None:
            xform = None
            #if to_print:
            #    print('Applying untransformed linear trend')
        elif int(measure_id) in [5, 17, 18]:
            xform = 'logit'
            #if to_print:
            #    print('Applying logit transformed trend')
        else:
            xform = 'log'
            #if to_print:
            #    print('Applying log transformed trend')
...
#predictions, status = fill_dismod_trend(MEID, MODEL_VERSION, RELEASE, LOC)
